...
function altsys_adminmenu_hack_ft(): void
{
    ...
    if (ALTSYS_ADMINMENU_HACK_2COL == $altsysModuleConfig['adminmenu_hack_ft']) {
        altsys_adminmenu_hack_ft_2col_x20();
    } elseif (ALTSYS_ADMINMENU_HACK_NOIMG == $altsysModuleConfig['adminmenu_hack_ft']) {
        altsys_adminmenu_hack_ft_noimg_x20();
    } elseif (ALTSYS_ADMINMENU_HACK_XCSTY == $altsysModuleConfig['adminmenu_hack_ft']) {
...
function altsys_adminmenu_hack_ft_2col_x20(): void
{
    ...
    if (!empty($altsys_adminmenu_ft_hacked)) {
        if (ALTSYS_ADMINMENU_HACK_2COL == $altsys_adminmenu_ft_hacked) {
            // skip
...
function altsys_adminmenu_hack_ft_noimg_x20(): void
{
    ...
    if (!empty($altsys_adminmenu_ft_hacked)) {
        if (ALTSYS_ADMINMENU_HACK_NOIMG == $altsys_adminmenu_ft_hacked) {
            // skip
...
function altsys_adminmenu_hack_ft_xcsty_x20(): void
{
    ...
    if (!empty($altsys_adminmenu_ft_hacked)) {
        if (ALTSYS_ADMINMENU_HACK_XCSTY == $altsys_adminmenu_ft_hacked && empty($altsys_adminmenu_dv_updated)) {
            // skip
    ...
    foreach ($mids as $mid) {
        ...
        $newline = "\n"
                    ...
                    . '" style="text-align:'
                    . _GLOBAL_LEFT
                    . ';background-color:#CCC;" title="'
                    . $module->getVar('dirname')
                    . '"><a id="adminmenu_ftpoint'
                    . $mid
                    . '" href="javascript:void(0);" onclick="submenuToggle('
                    . $mid
                    . ');">+</a> '
                    . $newline
                    . '>'
                    . $module->getVar('name')
                    . '</a></div><div id="adminmenu_ftsub'
                    . $mid
...
function altsys_adminmenu_save_x20($xoops_admin_vars): void
{
    ...
    if (in_array(@$xoops_admin_vars['altsys_adminmenu_ft_hacked'], [ALTSYS_ADMINMENU_HACK_NOIMG, ALTSYS_ADMINMENU_HACK_XCSTY], true)) {
        $output .= '

if( is_object( @$GLOBALS["xoopsModule"] ) && empty( $not_inside_cp_functions ) ) {
    $mid_tmp = $GLOBALS["xoopsModule"]->getVar("mid") ;
    if( $mid_tmp == 1 && @$_GET["fct"] == "preferences" && @$_GET["op"] == "showmod" && ! empty( $_GET["mod"] )) {
    $mid_tmp = $_GET["mod"];
    }
